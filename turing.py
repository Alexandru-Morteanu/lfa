    ...
    try:
        with open(filename, 'r', encoding='utf-8') as file:
            for line_num, line in enumerate(file, 1):
                ...
                # Section headers like [states], [rules], etc.
                ...
                if current_section == 'states':
                    ...
                    # Add to both input and tape alphabet
                    ...
                    # Format: q0 0 -> q1 X R
                    if '->' not in line:
                        print(f"Invalid rule at line {line_num}: {line}")
                        continue
                    ...
                    if len(left) != 2 or len(right) != 3:
                        print(f"Invalid rule format at line {line_num}: {line}")
                        continue

                    from_state, read_symbol = left
                    to_state, write_symbol, direction = right

                    tm.add_transition(from_state, read_symbol, to_state, write_symbol, direction)
                    # Add encountered symbols to tape alphabet
                    tm.tape_alphabet.update([read_symbol, write_symbol])
